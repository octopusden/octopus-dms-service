...
interface DmsServiceFeignClient {
    ...
    fun getComponents(
        @QueryMap filter: ComponentRequestFilter,
    ): ComponentsDTO
    ...
    @RequestLine(
        "POST rest/api/3/components/{component-name}/versions/{version}/artifacts/{artifact-id}?fail-on-already-exists={fail-on-already-exists}",
    )
    @Headers("Content-Type: application/json")
    ...
    @Headers("Content-Type: application/json")
    fun findArtifact(artifactCoordinates: ArtifactCoordinatesDTO): ArtifactDTO
