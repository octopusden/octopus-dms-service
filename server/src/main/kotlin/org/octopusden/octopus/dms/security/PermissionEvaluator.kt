...
class PermissionEvaluator(
    ...
    fun hasPermissionByComponent(componentName: String) =
        hasGroup(
    ...
    private fun hasGroup(
        user: User,
        componentName: String,
        readSecurityGroups: List<String>,
    ): Boolean {
        if (log.isTraceEnabled) {
            log.trace(
                "Check component '$componentName' read security groups $readSecurityGroups in '${user.username}' groups ${user.groups}'",
            )
        }
        ...
        return (group != null).also {
            if (log.isDebugEnabled) {
                log.debug(
                    "User '${user.username}' was${if (it) "" else " not"} granted to access component '$componentName' by group '$group'",
                )
            }
        }
    }
